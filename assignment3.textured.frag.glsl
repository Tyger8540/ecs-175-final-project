...
vec3 shadeAmbientLight(Material material, AmbientLight light) {
    ...
    return light.color * light.intensity * material.kA * texture(material.map_kD, texture_coords).rgb;
}
...
vec3 shadeDirectionalLight(Material material, DirectionalLight light, vec3 normal, vec3 eye, vec3 vertex_position) {
    ...
    result += LN * light.color * light.intensity * material.kD * texture(material.map_kD, texture_coords).rgb;

    // Specular
    vec3 R = reflect(L, N);
    result += pow( max(dot(R, V), 0.0), material.shininess * texture(material.map_nS, texture_coords).r) * light.color * light.intensity * material.kS;
...
void main() {
    ...
    vec3 normal = texture(u_material.map_norm, texture_coords).rgb;
    normal = normal * 2.0 - 1.0;  // make sure normals are between [-1, 1]
    ...
    for(int i = 0; i < MAX_LIGHTS; i++) {
        ...
        light_contribution += shadeDirectionalLight(u_material, u_lights_directional[i], normal, u_eye, frag_pos);
        light_contribution += shadePointLight(u_material, u_lights_point[i], normal, u_eye, frag_pos);
    }
